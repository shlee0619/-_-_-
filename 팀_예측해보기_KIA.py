...
def summarize_data():
    ...
    hitter_data_grouped = hitter_data[hitter_data['팀명'] == 'KIA'].drop(columns=['팀명'])
    pitcher_data_grouped = pitcher_data[pitcher_data['팀명'] == 'KIA'].drop(columns=['팀명'])
    defense_data_grouped = defense_data[defense_data['팀명'] == 'KIA'].drop(columns=['팀명'])
    ...
    print(merged)
    pd.DataFrame(merged).to_csv("./data/KIA_결합_기록2013.csv", index=False, encoding='utf-8-sig')
    return merged
...
print('예측된 승률:', predicted_win_rate)
