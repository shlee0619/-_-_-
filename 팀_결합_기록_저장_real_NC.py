...
def summarize_data():
    # SSG 팀 데이터 필터링
    team_data_grouped = team_data[team_data['팀명'] == 'LG'].drop(columns=['팀명'])  # '팀명' 열 삭제
    hitter_data_grouped = hitter_data[hitter_data['팀명'] == 'LG'].drop(columns=['팀명'])
    pitcher_data_grouped = pitcher_data[pitcher_data['팀명'] == 'LG'].drop(columns=['팀명'])
    defense_data_grouped = defense_data[defense_data['팀명'] == 'LG'].drop(columns=['팀명'])
    ...
    print(merged)
    pd.DataFrame(merged).to_csv("./data/LG_결합_기록.csv", index=False, encoding='utf-8-sig')
    return merged
